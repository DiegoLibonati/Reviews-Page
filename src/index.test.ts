...
import { REVIEWS_MOCK } from "./tests/constants/constants";
...
test("It must render the title of the reviews.", () => {
  const titleReviews = screen.getByRole("heading", { name: /client reviews/i });
...
test("It should render the next, previous and surprise buttons.", () => {
  ...
  const btnRandom = screen.getByRole("button", { name: /btn random review/i });
